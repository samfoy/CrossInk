 ...
 * (POST <base>/koreader/plugin/page-stats).
 *
 ...
 * sessions. This store lets CrossInk feed that same channel.
 *
 * One event is appended per qualifying forward page read (reusing the dwell
 * time CrossInk already measures for its own reading-pace stats). Events are
 * grouped per document hash. The buffer is flushed to disk with the same
 * debounce as the book's reading stats (never once-per-page — see AGENTS.md),
...
// To stay robust against CrossInk's reflowable per-section pagination we encode
// overall book progress as page/totalPages: `page` is the overall progress in
// basis points (0..10000) and `totalPages` is fixed at PROGRESS_SCALE, so the
// server derives endProgress = page/totalPages*100 == the exact reading %.
